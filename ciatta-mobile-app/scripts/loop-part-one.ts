...
function serveFunctions(): Promise<{ stop: () => void }> {
  return new Promise((resolve, reject) => {
...
async function post(url: string, serviceKey: string): Promise<{ processed: boolean; threads?: number; insights?: number }> {
  const res = await fetch(`${url}/functions/v1/intelligence`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${serviceKey}`, 'Content-Type': 'application/json' },
    body: '{}',
  });
  const body = (await res.json()) as { processed: boolean; threads?: number; insights?: number; error?: string };
...
// empty list: every read below is of rows the run was expected to write.
function rows<T>(result: { data: T[] | null; error: { message: string } | null }, what: string): T[] {
