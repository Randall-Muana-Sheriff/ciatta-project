...
- `20260922100100_insights` (research_refs, insights, and `thread_evidence.research_ref_id`) landed later the same day once the tool permission layer was allowed to deploy. All three migrations are live with their exact file versions.
...
- **Scheduler configured:** Vault now holds `service_role_key`, stored from inside the project by a one off function so the key never left it. Note for anyone reading `baselines_tick` or `intelligence_tick`: the platform hands edge functions a newer `sb_secret_` key (41 characters), not the legacy JWT, and the gateway accepts it as a bearer with `verify_jwt` on; that is the value both functions compare against and the value Vault holds. `scheduler_health` reads configured for all three drains.
- **Closing smoke test on live:** one intelligence job queued for the founder's own account, one `intelligence_tick(1)`: pg_net response 200 `{"processed":true,"threads":0,"insights":0}`, function log `threads upserted 0`, `insights written 0`, every withheld reason 0, job `done` after one attempt with no error. Zero because live holds no health rows yet; the path itself (Vault key, pg_net, gateway, function, RPCs, complete) is proven.
- **Cleanup done:** the SQL guard dropped, the `understanding_engine_key` secret deleted, `configure-scheduler` redeployed as an inert 410 stub. **Left for the dashboard or CLI** (the connector cannot delete functions): delete `configure-scheduler`, `understanding-engine` and `provider-search`; turn on leaked password protection.
